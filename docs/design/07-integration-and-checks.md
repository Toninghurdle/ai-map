...
- **Mounting.** Mount the map in a client component. The script expects these IDs: `map`, `map-wrap`, `chart`, `grid-top`, `grid-left`, `census`, `layers`, `lensbar`, `lb-chips`, `lb-def`, `find`, `find-list`, `links-btn`, `home-btn`, `panel`, `tip`, `kbd-hint`. These are optional and skipped if absent: `meta`, `chart-foot`, `index-cols`, `theme-btn`, and the `.lede` paragraph. The page wrapper around all of it must also carry the class `page`, because the stacked-layout rule `body.map-stack.has-panel .page` is what adds the bottom padding that lets the map scroll clear of the phone bottom sheet; without it the page is about 880px too short on a phone and nothing says why. The site may carry its own class alongside it. The script is a singleton (module-level state, one set of IDs), so it supports one live map per page. For a small map on detail pages, use a static SVG crop of the island rendered on the server, not a second instance; making it multi-instance means wrapping the state in a factory that takes a root element, which is a refactor, not a setting.
