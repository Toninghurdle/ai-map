...
- **Expected: the panel isn't built yet (task 3).** A separate numbered list. The site mounts the map with `panel: false` and `reserveRight: 430` (`apps/web/components/FieldMapMount.tsx`), and `apps/web/app/(map)/layout.tsx` renders an empty `#panel`; the reference draws its own built-in panel. So every state that opens a panel will differ in the right-hand strip, and the camera frames the lit set differently on both sides. Put those differences here, not in the main list, and be specific about which ones are the panel gap rather than sweeping a whole state into this bucket. Anything in the panel strip that is *not* explained by task 3, say so, and rank it in the main list.
